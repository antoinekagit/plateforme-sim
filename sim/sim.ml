...
module EnvLogSfcPar = struct
  ...
  let clear_logs = lift_ext (EnvLogSfc.lift_ext EnvLog.clear)
...
let main_repl =
  let rec loop : 'id. (Sim.t,'id) mconc -> (Sim.t,'id) mconc =
    ...
    | "next" -> 
       let mcsim = mcsim >>=: period in
       let ((((sim, param), accs), logs), env) = extract mcsim in
       let () = display_repl sim param accs logs env in
       loop mcsim
    | "stop" -> mcsim
    | other -> print_endline "unknown command" ; loop mcsim
  in
